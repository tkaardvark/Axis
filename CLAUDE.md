...
- **Design system tokens & primitives:** `client/src/index.css` defines spacing (`--space-xs`..`--space-3xl`), border-radius (`--radius-sm`..`--radius-pill`), type scale (`--text-xs`..`--text-3xl`), font weights, shadows, and transitions. `client/src/App.css` exposes shared component classes — `.page-header`/`.page-subtitle` (page-level), `.ds-card`, `.ds-section-header`, `.ds-section-title`, `.ds-section-subtitle` (or legacy `.section-subtitle`), `.ds-eyebrow`, `.ds-badge`. New components should prefer these over redefining the same look in per-page CSS.
